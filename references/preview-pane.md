...
从行操作、卡片操作、记录菜单、右键菜单、长按菜单或上下文操作打开预览时，必须同时执行 `references/row-contextual-actions.md`。预览 owner 负责 previewPaneState、预览目标、只读请求和关闭返回；行操作 owner 负责 `rowActionState`、recordIdentity、sourceSnapshot、navigationBinding 和旧行防护。
