...
保存栏、sticky footer、Dialog footer、Drawer footer 或移动端底部操作与表单字段、字段组、错误文本、帮助文本和焦点目标发生布局关系时，必须同时执行 `references/form-layout-field-groups.md`，并声明 `formLayoutState`。本 owner 负责保存/取消意图和操作区状态；表单布局 owner 负责字段区域留白、错误定位、footer 遮挡避让、虚拟键盘和移动端单列下的可达性。
