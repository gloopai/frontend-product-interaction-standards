...
- 穿梭框、双列表选择、资源分配、权限分配、菜单授权、角色授权、成员分组、数据范围授权和关联资源批量绑定必须同时执行 `references/transfer-assignment-lists.md`，并声明 `assignmentTransferState`；本文件只负责 chip/token 输入，多集合移动、左右列表、选中桶、diff 摘要和保存边界归分配列表 owner。
