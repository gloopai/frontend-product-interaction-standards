...
金额、余额、额度、用量、阈值、单价、折扣、税率、汇率、预算、库存、席位数、数量和计量周期等数字字段必须同时执行 `references/numeric-amount-inputs.md`。权限 owner 负责证明数字、单位、历史值、错误和旧缓存无泄露；数字 owner 负责 `numericInputState`、permissionBoundary、submitSnapshotPolicy、旧 aria-label/tooltip/校验结果清理和移动端承载。

行操作、记录操作、上下文操作、行更多菜单、卡片操作、右键菜单、长按菜单和单条记录动作必须同时执行 `references/row-contextual-actions.md`。权限 owner 负责记录、动作、菜单、禁用原因和旧缓存的无泄露；行操作 owner 负责 `rowActionState`、availabilityMap、permissionBoundary、requestIdentity、resultReceipt 和权限变化后的动作收敛。
