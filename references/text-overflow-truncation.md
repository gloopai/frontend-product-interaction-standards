...
表单布局中的长 label、长帮助、长错误、长 placeholder、组合字段、单位、计数和跨列字段溢出必须同时执行 `references/form-layout-field-groups.md`，并声明 `formLayoutState`。文本溢出 owner 负责折行、截断和全文恢复；表单布局 owner 负责字段组、跨列、相邻字段挤压、防遮挡和移动端单列排列。
