...
本 Skill 当前包含以下 Dialog、四向抽屉、折叠面板与 Disclosure、可搜索单选 Select、多选/标签输入与 Tokenized Input、对象/资源/成员选择器、穿梭框/分配列表与授权资源选择、选择控件与开关、树形结构与级联、表单、表单布局/字段分组与响应式排列、数字/金额/比例/配额输入、条件字段与依赖输入、字段说明/帮助文本与占位提示、页面级表单操作栏与保存区、复杂编辑器和构建器、数据表格、表格列设置/列布局与密度、批量操作与批处理动作、排序与重排、查询条件与筛选、关键词搜索输入、列表结果控制、卡片列表与卡片式结果、搜索与命令面板、日期时间与时区、信息展示与详情页、详情预览面板、密钥、令牌与敏感凭证、Webhook/集成连接与回调配置、计费/套餐/订阅与发票、图表与可视化、概览页与仪表盘首页、导出/下载与结果产物、文件与媒体资产管理、用户侧附件与内容提交上传、页面操作栏与列表工具栏、保存视图与布局预设、设置/偏好与配置页、分步流程与配置向导、Tab 视图导航、页面标题区与 Page Header、页面内容区与 Section 布局、管理台 App Shell 与导航外框、导航与路由、会话、认证与重新认证、记录新增/编辑承载面、按钮、行操作与上下文操作、浮层菜单与提示、危险操作与恢复、回收站/软删除/归档恢复与保留期、审批与审核工作流、状态流转与记录生命周期、成员、邀请与团队访问管理、权限/租户与可见性、异步任务与任务中心、审计日志与操作历史、上传与导入、反馈状态、空态/无结果与首次使用引导、全局反馈、跨端适配和管理台治理核心要求：
...
- 对象、资源与成员选择器规范：见 `references/entity-resource-pickers.md`。约束 entityResourcePickerState、稳定身份、作用域、版本、展示快照、搜索结果、最近/收藏/推荐、不可绑定原因、跨范围可见且可绑定证明、权限无泄露、迟到请求、移动端选择承载和运行时未验证边界，避免把业务对象选择器当作普通 Select 或用 display label 作为身份。
- 穿梭框、分配列表与授权资源选择规范：见 `references/transfer-assignment-lists.md`。约束 assignmentTransferState、初始/草稿已分配集合、左右可见集合、选中桶、移动意图、eligibility、diff 摘要、保存策略、权限无泄露、树形半选、全选范围、迟到请求、移动端分配承载和运行时未验证边界，避免移动即保存、勾选即移动、当前页冒充全部候选或 selectedKeys 一把梭。
...
- 表单布局、字段分组与响应式排列规范：见 `references/form-layout-field-groups.md`。约束 formLayoutState、字段注册、组注册、视觉/DOM/Tab/读屏顺序、label/帮助/错误绑定、跨列策略、长内容溢出、底部操作栏避让、动态字段重排、移动端单列和运行时未验证边界，避免只用 CSS Grid、span、Form.Item 或截图伪装布局正确。
- 数字、金额、比例与配额输入规范：见 `references/numeric-amount-inputs.md`。约束 numericInputState、草稿/解析/提交值分离、单位/币种/倍率、精度/舍入、硬软边界、步进、粘贴、IME、权限无泄露和移动端键盘承载，避免 `Number(value)`、`parseFloat`、`input[type=number]` 或组件库 formatter 伪装成业务正确性。
- 条件字段与依赖输入规范：见 `references/conditional-fields-dependent-inputs.md`。约束 fieldDependencyState、依赖图、上游快照、下游清值/失效/重算、条件显隐、条件必填、隐藏值提交、候选失效、自动填充、权限无泄露和移动端承载。
- 字段说明、帮助文本与占位提示规范：见 `references/field-guidance-help-text.md`。约束 fieldGuidanceState、label、必填/选填、placeholder、帮助说明、单位、格式示例、空值说明、权限原因、错误关系、hover-only 替代路径和旧 ARIA 引用清理。
...
- 表格列设置、列布局与密度规范：见 `references/table-column-layout-density.md`。约束 tableColumnLayoutState、列显隐、顺序、宽度、固定、密度、草稿/应用/持久化分离、必显列、权限无泄露、重置语义、保存视图转交和移动端列配置承载。
- 批量操作与批处理动作规范：见 `references/bulk-actions-batch-operations.md`。约束 bulkActionState、选择快照、范围绑定、目标集合、可执行性、确认、请求身份、部分成功、结果回执、恢复、权限无泄露和移动端承载，避免当前可见行循环执行、范围混用、Toast-only 回执和旧选择继续提交。
...
- 乐观更新、撤销与回滚规范：见 `references/optimistic-update-undo.md`。约束 optimisticMutationState、pending/syncing/undoable/queued 非终态、undoPolicy、rollbackPolicy、reconciliationPolicy、幂等、迟到响应、权限无泄露、移动端弱网/离线恢复和运行时未验证边界。
- 快捷键与键盘命令规范：见 `references/keyboard-shortcuts-commands.md`。约束 keyboardShortcutState、scopeBinding、inputProtectionPolicy、conflictPolicy、discoverabilityPolicy、executionPolicy、系统/浏览器/读屏快捷键避让、移动端替代和运行时未验证边界。
...
- 页面标题区与 Page Header 规范：见 `references/page-header-title-area.md`。约束 pageHeaderState、页面身份、标题快照、副标题、状态摘要、标题区主操作、导航绑定、权限无泄露、移动端标题区和运行时未验证边界。
- 页面内容区与 Section 布局规范：见 `references/page-content-layout-sections.md`。约束 pageContentLayoutState、主内容区、Section/Card 注册、ownerHandoff、主滚动、Sticky/fixed 避让、内容密度、权限无泄露、移动端单列和运行时未验证边界。
...
- 行操作与上下文操作规范：见 `references/row-contextual-actions.md`。约束 rowActionState、动作目标、来源快照、旧行防护、动作可见性、菜单边界、请求身份、结果回执、权限无泄露、虚拟列表和移动端承载。
...
- 回收站、软删除、归档恢复与保留期规范：见 `references/trash-restore-retention.md`。约束 trashRestoreState、soft delete/archive/disable/permanent delete/legal hold 语义、恢复入口、保留期、永久删除、旧入口清理、权限无泄露、批量部分成功、任务中心和审计追溯。
...
- 空态、无结果与首次使用引导规范：见 `references/empty-first-run-zero-results.md`。约束 emptyStateDecision、空态原因、首次使用、筛选无结果、只读报表、权限空态、CTA 匹配、清空/重置筛选、权限无泄露和移动端承载，避免所有空态都写成“暂无数据”。
