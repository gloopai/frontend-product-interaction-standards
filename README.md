...
本 Skill 当前包含以下 Dialog、四向抽屉、可搜索单选 Select、表单、数据表格、查询条件与筛选、记录新增/编辑承载面、按钮、上传与导入、反馈状态、跨端适配和管理台治理核心要求：
