...
- 承载面决策会先判断任务复杂度、字段数量、选择器数量、风险等级、上下文对照、虚拟键盘、固定底部操作和可用视口，再决定居中 Dialog、Bottom Drawer、全屏 Drawer 或独立页。
- 短确认和无表单轻任务可保留居中 Dialog；中等复杂度任务移动端优先 Bottom Drawer；长表单、多步骤、高风险、强上下文编辑或需要稳定 URL / 刷新恢复的任务升级为全屏 Drawer 或独立页。
- 规范禁止把所有移动端 Dialog 一刀切改成 Bottom Sheet，也禁止把所有 Select 一刀切改成 Drawer。
- Select 承载面同时判断搜索需求、选项文本长度、禁用项/失效项、远程加载、错误重试、虚拟键盘、触控目标、外层剩余高度和底部操作避让。
